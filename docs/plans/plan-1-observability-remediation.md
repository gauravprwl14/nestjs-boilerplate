...
- [ ] **W2.3** Implement
  ...
  import { ErrorException } from '@errors/types/error-exception';
  import { serialiseErrorChain, type SerialisedErrorFrame } from '@errors/utils/cause-chain.util';
    ...
    return target => {
      ...
      while (current && current !== Object.prototype) {
        for (const key of Object.getOwnPropertyNames(current)) {
          ...
          Trace({ spanName: `${target.name}.${key}`, ...options })(proto, key, descriptor);
  ...
  describe('observability e2e', () => {
    ...
    it('redacts email and password from span attributes and events', async () => {
      await request(app.getHttpServer())
        ...
        .send({ email: 'a@x.com', password: 'hunter2' });
      const spans = exporter.getFinishedSpans();
