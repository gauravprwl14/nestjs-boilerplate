...
<!-- DOC-SYNC: Diagram updated on 2026-04-25. ArchivalModule and MockDataModule added under src/modules/. Please verify visual accuracy before committing. -->
...
| `ArchivalModule`    | `ArchivalController`    | `ArchivalService`, `PartitionRotationService`                                                 | `MultiDbService`, `ArchiveRegistryService` (from DatabaseModule) |
| `MockDataModule`    | `MockDataController`    | `MockDataService`                                                                             | `MultiDbService`, `ArchiveRegistryService` (from DatabaseModule) |
| `DatabaseModule`    | —                       | `DatabaseService` + per-aggregate `*DbService`s + `MultiDbService` + `ArchiveRegistryService` | per-aggregate `*DbRepository`s, `PrismaService.tenantScoped`     |
